...
!(async () => {
    let num = 0;
    let proxyIp = "129.226.100.212:8877"; //await getIp() // 获取本次代理ip
...
async function getUrls(options) {
    ...
    try {
        data = await got
            .get("https://p.m.jd.com/cart/cart.action?fromnav=1&sceneval=2&jxsid=16371654022816454477", options)
            .text();
    } catch (e) {
        console.log("请求失败：" + e);
    }
...
async function changeLinks(urls) {
    ...
    for (let url of urls) {
        ...
        const { data } = await got
            .post("https://www.dgrlm.com/qcypopen/open/v1/qcSmartChain", {
                json: params,
                timeout: {
                    request: 10000,
                },
...
async function getIp() {
    const ipApi =
        "http://webapi.http.zhimacangku.com/getip?num=1&type=1&pro=110000&city=110200&yys=100026&port=1&time=3&ts=0&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions=";
    // const ipApi ='http://api.89ip.cn/tqdl.html?api=1&num=20&port=&address=&isp=';
    let ip = await got
        .get(ipApi, {
            timeout: {
                request: 10000,
...
async function browse(urls, cks, proxyIp) {
    ...
    await page.on("request", (interceptedRequest) => {
        ...
        if (
            ...
            interceptedRequest.url().endsWith(".dpg") ||
